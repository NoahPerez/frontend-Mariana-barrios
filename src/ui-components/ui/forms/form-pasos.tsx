...
const FormPasos = forwardRef<HTMLFormElement, TProps>(
    ({ className, button = 'Enviar' }, ref) => {
        ...
        const onSubmit: SubmitHandler<IFormValues> = async (form) => {
            ...
            let url = `${API_URL}/formulario-pasos`;
            ...
            console.log({ data2 });
            reset()
            setMessage("Muchas gracias!, pronto nos pondremos en contacto contigo.");
        };
        return (
            <form
                className={`${clsx(className)} tw-flex tw-flex-col tw-gap-4 tw-w-full `}
                ref={ref}
